// src/routes/ended-auctions-page.tsx
  ...
  useEffect(() => {
    async function fetchAuctions() {
      try {
        ...
        const response = await AuctionService.getEndedAuctions(page, 20);
        ...
        setError(
          err.message ||
          "Failed to load ended auctions. Please try again later."
        );
      } finally {
        setLoading(false);
      }
      ...
      <div className="space-y-5">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground sm:text-base">
            Browse past auctions and see final sales results.
          </p>
        </div>
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-center py-12">
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            <div>
              <p className="font-semibold">No ended auctions found</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-5">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground sm:text-base">
          Browse past auctions and see final sales results. {total > 0 && `(${total} ended)`}
        </p>
      </div>
...
export function CarCard({ car }: { car: AuctionCar }) {
  ...
  return (
    <Link to={`/auctions/${car.id}`} className="block">
      <Card className="flex flex-col overflow-hidden transition hover:-translate-y-0.5 hover:shadow-lg opacity-90 hover:opacity-100">
        <div className="relative aspect-[16/9] w-full overflow-hidden grayscale-[50%] hover:grayscale-0 transition-all duration-300">
          <img
            src={car.imageUrl}
            alt={car.title}
          ...
          <div className="absolute left-3 top-3">
            {car.isSold ? (
              <Badge className="bg-emerald-600 hover:bg-emerald-700 text-white">SOLD</Badge>
            ) : (
